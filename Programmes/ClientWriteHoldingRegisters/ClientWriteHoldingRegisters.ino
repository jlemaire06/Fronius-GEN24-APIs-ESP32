// ClientWriteHoldingRegisters.ino

/***********************************************************************************
  Modbus TCP client (master) to write asynchronously contiguous holding registers in 
  a Fronius GEN24 inverter (slave), using an ESP32 as Wifi station in a local network.
...
void setup()
{
  ...
  uint16_t regAddress = 40232; // WMaxLimPct = power output limit (%)
  uint16_t nbRegisters = 5; // nb words 
  ...
  // Response: serverID=1, FC=16, Token=00000A4C, length=6:
  // 01 10 9D 28 00 05 
}
